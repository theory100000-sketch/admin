...
function telBotVersionKey(){ return `${TEL_REDIS_SAFE_PREFIX}__version`; }
function telBotActiveDataKey(){ return `${TEL_REDIS_SAFE_PREFIX}__active_data_key`; }
...
function telBotDataTimestamp(data, fallbackMs=0){
  const candidates = [
    data?.sync?.updatedAt, data?.updatedAt, data?.actualizadoEn,
    ...(Array.isArray(data?.clubes) ? data.clubes.flatMap(c=>[c?.actualizadoEn,c?.creadoEn]) : []),
    ...(Array.isArray(data?.jugadores) ? data.jugadores.flatMap(j=>[j?.actualizadoEn,j?.registradoEn]) : [])
  ];
  let best = Number(fallbackMs || 0);
...
async function telBotInitialSync(){
  if(!telBotRedis) return {mode:'local-only'};
  const dataKey = await telBotResolveDataRedisKey(true);
  const localText = fs.existsSync(DATA_FILE) ? fs.readFileSync(DATA_FILE,'utf8') : '';
  let localData = {};
  try{ localData = JSON.parse(localText || '{}'); }catch(error){}
  let localMtime = 0;
  try{ localMtime = fs.statSync(DATA_FILE).mtimeMs; }catch(error){}

  const stored = await telBotRedis.get(dataKey);
  if(stored === null || stored === undefined){
    telBotStorageReady = true;
    await telBotPersistData(localText || JSON.stringify(telNormalizarDatosParaWeb(localData),null,2));
    return {mode:'local-seed', localAt:telBotDataTimestamp(localData,localMtime)};
  }

  const remoteText = typeof stored === 'string' ? stored : JSON.stringify(stored,null,2);
  let remoteData = {};
  try{ remoteData = JSON.parse(remoteText || '{}'); }catch(error){}
  const localAt = telBotDataTimestamp(localData,localMtime);
  const remoteAt = telBotDataTimestamp(remoteData,0);

  // La copia más reciente gana. Esto evita que una data antigua de Upstash
  // borre clubes creados recientemente en el ordenador donde funciona el bot.
  if(localText && localAt > remoteAt + 1000){
    telBotStorageReady = true;
    await telBotPersistData(localText);
    return {mode:'local-newer',localAt,remoteAt};
  }

  fs.writeFileSync(DATA_FILE, remoteText, 'utf8');
  const remoteVersionRaw = await telBotRedis.get(telBotVersionKey());
  telBotRemoteVersion = String(remoteVersionRaw || '');
  return {mode:'remote-newer',localAt,remoteAt};
}
...
async function telPrepareBotStorage(){
  try{
    ...
    if(!telBotRedis){
      telBotStorageReady = true;
      console.error('[bot-storage] SIN SINCRONIZACIÓN: faltan UPSTASH_REDIS_REST_URL o UPSTASH_REDIS_REST_TOKEN en el .env del BOT.');
      console.error('[bot-storage] Discord funcionará, pero los cambios NO llegarán a Vercel.');
      return;
    }

    const result = await telBotInitialSync();
    telBotStorageReady = true;
    const preparedData = cargarDatos();
    await guardarDatos(preparedData);
    ...
    let redisHost='desconocido';
    try{ redisHost=new URL(telBotRedisUrl).host; }catch(error){}
    console.log(`[bot-storage] SINCRONIZACIÓN ACTIVA | host=${redisHost} | prefijo=${TEL_REDIS_SAFE_PREFIX} | clave=${await telBotResolveDataRedisKey(true)} | inicio=${result.mode}`);
    console.log('[web-sync] Cada cambio de un comando se publica automáticamente en la web.');
  }catch(error){
    telBotStorageReady = true;
    console.error('[bot-storage] ERROR DE SINCRONIZACIÓN:', error.message || error);
    console.error('[bot-storage] Revisa que el BOT tenga la misma URL, token y prefijo que Vercel.');
  }
}
