...
/* ============================================================
   TEL FIX DEFINITIVO CALENDARIO - MISMO CRUCE MAXIMO 2
   Este bloque trabaja sobre el calendario ACTUAL:
   - NO borra partidos.
   - NO regenera todo a ciegas.
   - Revisa jornadas en orden.
   - Si un cruce ya salió 2 veces antes, cambia emparejamientos
     dentro de ESA MISMA JORNADA para que no falte ningún equipo.
   - También filtra la respuesta JSON para que el fallo no vuelva a
     aparecer por funciones antiguas del server.
   ============================================================ */
function telStrictCalNorm(value){
  return String(value || '')
    .toLowerCase()
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g,'')
...
function telStrictCalIsCup(comp){
  const txt = telStrictCalNorm(`${comp?.nombre || ''} ${comp?.tipo || ''} ${comp?.formato || ''} ${comp?.formatoNombre || ''}`);
  return txt.includes('copa') || txt.includes('elimin');
}
function telStrictCalNameFromTeam(team){
  return String(team?.nombre || team?.clubNombre || team?.nombreVisual || team?.name || '').trim();
}
function telStrictCalSide(match, side){
  const p = side === 'local' ? 'local' : 'visitante';
  const cap = side === 'local' ? 'Local' : 'Visitante';
  const name = String(
    match?.[`${p}Nombre`] ||
    match?.[side === 'local' ? 'nombreLocal' : 'nombreVisitante'] ||
    match?.[side === 'local' ? 'equipoLocal' : 'equipoVisitante'] ||
    match?.[p]?.nombre ||
    match?.[p]?.clubNombre ||
    match?.[`${p}Team`]?.nombre ||
    match?.[`${p}Equipo`]?.nombre ||
    match?.[`${p}Club`]?.nombre ||
    ''
  ).trim();
  const slot = String(match?.[`${p}SlotId`] || match?.[`${p}Id`] || match?.[`${p}ClubId`] || match?.[p]?.slotId || match?.[p]?.id || match?.[p]?.clubId || name).trim();
  const logo = String(match?.[`${p}Logo`] || match?.[`${p}Escudo`] || match?.[`logo${cap}`] || match?.[`escudo${cap}`] || match?.[p]?.escudoUrl || match?.[p]?.logoUrl || match?.[p]?.logo || match?.[p]?.escudo || '').trim();
  return {
    name,
    key: telStrictCalNorm(name),
    slotId: slot,
    id: slot,
    clubId: String(match?.[`${p}ClubId`] || match?.[p]?.clubId || slot).trim(),
    logo,
    obj: match?.[p] || match?.[`${p}Team`] || match?.[`${p}Equipo`] || match?.[`${p}Club`] || {}
  };
}
function telStrictCalTeamFromComp(team){
  const name = telStrictCalNameFromTeam(team);
  const slot = String(team?.slotId || team?.id || team?.clubId || team?.idClub || name).trim();
  const logo = String(team?.escudoUrl || team?.logoUrl || team?.logo || team?.escudo || '').trim();
  return {
    name,
    key: telStrictCalNorm(name),
    slotId: slot,
    id: slot,
    clubId: String(team?.clubId || team?.idClub || slot).trim(),
    logo,
    obj: team || {}
  };
}
function telStrictCalPairKey(a,b){
  const x = telStrictCalNorm(typeof a === 'string' ? a : a?.name);
  const y = telStrictCalNorm(typeof b === 'string' ? b : b?.name);
  if(!x || !y || x === y) return '';
  return [x,y].sort().join(' vs ');
}
function telStrictCalPlayed(match){
  if(!match) return false;
  const score = [match.localGoles, match.visitanteGoles, match.golesLocal, match.golesVisitante].some(v =>
...
function telStrictCalApplySide(match, side, team){
  const p = side === 'local' ? 'local' : 'visitante';
  const cap = side === 'local' ? 'Local' : 'Visitante';
  const name = String(team.name || '').trim();
  const slot = String(team.slotId || team.id || team.clubId || name).trim();
  const logo = String(team.logo || team.obj?.escudoUrl || team.obj?.logoUrl || team.obj?.logo || team.obj?.escudo || '').trim();
  const obj = {
    ...(team.obj || {}),
    id: slot,
    slotId: slot,
...
function telStrictCalSortMatches(matches){
...
function telStrictCalActiveTeams(comp){
  const out = [];
  const seen = new Set();
  for(const raw of (Array.isArray(comp?.equipos) ? comp.equipos : [])){
    const flags = raw?.historico || raw?.equipoHistorico || raw?.fixtureHistorico || raw?.excluidoClasificacion || raw?.eliminadoDeCompeticion || raw?.sacadoDeCompeticion || raw?.removidoDeCompeticion || raw?.inactivoCompeticion || raw?.activo === false || raw?.active === false;
    if(flags) continue;
    const t = telStrictCalTeamFromComp(raw);
...
function telStrictCalGetRoundTeams(roundMatches, comp){
  const map = new Map();
  for(const match of roundMatches){
    for(const side of ['local','visitante']){
      const t = telStrictCalSide(match, side);
      if(t.key && !map.has(t.key)) map.set(t.key, t);
    }
  }
  if(map.size === roundMatches.length * 2) return Array.from(map.values());

  // Si la jornada ya venía rota con un equipo repetido/faltante, usa participantes activos.
  const active = telStrictCalActiveTeams(comp);
  return active.length >= 2 ? active.slice(0, roundMatches.length * 2) : Array.from(map.values());
}
function telStrictCalOriginalPairSet(roundMatches){
  const set = new Set();
  for(const m of roundMatches){
    const a = telStrictCalSide(m,'local');
    const b = telStrictCalSide(m,'visitante');
    const key = telStrictCalPairKey(a,b);
    if(key) set.add(key);
  }
  return set;
}
function telStrictCalFindMatching(teams, previousCounts, originalPairs){
  const memo = new Map();

  function rec(remainingKeys){
    if(!remainingKeys.length) return {pairs:[], score:0};

    const memoKey = remainingKeys.join('|');
    if(memo.has(memoKey)) return memo.get(memoKey);

    const firstKey = remainingKeys[0];
    const first = teams.find(t=>t.key === firstKey);
    let best = null;

    for(let i=1; i<remainingKeys.length; i++){
      const otherKey = remainingKeys[i];
      const other = teams.find(t=>t.key === otherKey);
      if(!first || !other) continue;

      const pairKey = telStrictCalPairKey(first, other);
      if(!pairKey) continue;
      if((previousCounts.get(pairKey) || 0) >= 2) continue;

      const rest = remainingKeys.filter(k=>k !== firstKey && k !== otherKey);
      const sub = rec(rest);
      if(!sub) continue;

      const keepScore = originalPairs.has(pairKey) ? 100 : 0;
      const score = sub.score + keepScore;
      const candidate = {pairs:[[first, other], ...sub.pairs], score};
  ...
  return rec(teams.map(t=>t.key).sort());
}
function telStrictCalAssignPairsToMatches(roundMatches, pairs){
  const remainingPairs = pairs.slice();
  const changes = [];

  for(const match of roundMatches){
    const oldLocal = telStrictCalSide(match,'local');
    const oldAway = telStrictCalSide(match,'visitante');
    const oldPair = telStrictCalPairKey(oldLocal, oldAway);

    let idx = remainingPairs.findIndex(([a,b])=>telStrictCalPairKey(a,b) === oldPair);
    if(idx < 0) idx = 0;

    const [a,b] = remainingPairs.splice(idx,1)[0] || [];
    if(!a || !b) continue;

    const before = `${oldLocal.name} vs ${oldAway.name}`;

    // Si el par es el mismo, conserva orientación original.
    if(telStrictCalPairKey(a,b) === oldPair){
      continue;
    }
    ...
    const after = `${telStrictCalSide(match,'local').name} vs ${telStrictCalSide(match,'visitante').name}`;
    match.reparadoCruceMasDeDos = true;
    match.cruceAnterior = before;
    match.cruceNuevo = after;
    changes.push({
...
function telStrictCalValidate(comp){
  const problems = [];
  ...
  for(const m of (Array.isArray(comp?.partidos) ? comp.partidos : [])){
    ...
    const key = telStrictCalPairKey(telStrictCalSide(m,'local'), telStrictCalSide(m,'visitante'));
    if(key) counts.set(key, (counts.get(key) || 0) + 1);
  }

  for(const [pair,count] of counts.entries()){
    if(count > 2) problems.push({tipo:'cruce_repetido_mas_de_2', cruce:pair, veces:count});
  }
  ...
  for(const [j,matches] of byRound.entries()){
    ...
    for(const m of matches){
      for(const side of ['local','visitante']){
        const t = telStrictCalSide(m, side);
        if(!t.key) continue;
        used.set(t.key, (used.get(t.key) || 0) + 1);
      }
    }
...
function telStrictCalFixCompetition(comp){
  if(!comp || telStrictCalIsCup(comp) || !Array.isArray(comp.partidos)) return comp;

  const byRound = new Map();
  for(const match of telStrictCalSortMatches(comp.partidos)){
    const j = Number(match.jornada || match.round || 0);
    if(!byRound.has(j)) byRound.set(j, []);
    byRound.get(j).push(match);
  }
  ...
  for(const jornada of jornadas){
    ...
    const originalPairs = telStrictCalOriginalPairSet(roundMatches);
    const currentHasBadPair = roundMatches.some(m=>{
      const key = telStrictCalPairKey(telStrictCalSide(m,'local'), telStrictCalSide(m,'visitante'));
      return key && (previousCounts.get(key) || 0) >= 2;
    });
    ...
    roundMatches.forEach(m=>{
      ['local','visitante'].forEach(side=>{
        const t = telStrictCalSide(m, side);
        if(t.key) usedNow.set(t.key, (usedNow.get(t.key) || 0) + 1);
      });
    });
    const currentHasRepeatedTeam = Array.from(usedNow.values()).some(n=>n>1);

    if(currentHasBadPair || currentHasRepeatedTeam){
      const teams = telStrictCalGetRoundTeams(roundMatches, comp);
      const matching = telStrictCalFindMatching(teams, previousCounts, originalPairs);

      if(matching && matching.pairs && matching.pairs.length === roundMatches.length){
        changes.push(...telStrictCalAssignPairsToMatches(roundMatches, matching.pairs));
      }else{
        unresolved.push({
          jornada,
          motivo:'No existe emparejamiento posible en esta jornada sin crear un tercer cruce.',
          partidos:roundMatches.map(m=>`${telStrictCalSide(m,'local').name} vs ${telStrictCalSide(m,'visitante').name}`)
        });
      }
    }

    // Contar la jornada final ya corregida.
    for(const m of roundMatches){
      const key = telStrictCalPairKey(telStrictCalSide(m,'local'), telStrictCalSide(m,'visitante'));
  ...
  comp.partidos = telStrictCalSortMatches(comp.partidos);
  comp._strictCrucesCambiados = changes;
  comp._strictCrucesSinSolucion = unresolved;
  comp._strictCrucesProblemas = telStrictCalValidate(comp);
...
function telStrictCalFixData(data){
  if(!data || typeof data !== 'object') return data;
  const comps = data.competiciones || data.ligas || data.torneos || [];
  comps.forEach(comp=>telStrictCalFixCompetition(comp));
  if(typeof telCompTableApply === 'function'){
    try{ telCompTableApply(data); }catch(error){}
  }
  return data;
}
async function telStrictCalHardSave(data){
  const payload = JSON.stringify(data, null, 2);
  try{
    if(typeof telNativeFs !== 'undefined' && typeof telRuntimePath === 'function'){
      telNativeFs.writeFileSync(telRuntimePath(DATA_FILE), payload, 'utf8');
    }else{
  ...
  try{
    if(typeof telRedis !== 'undefined' && telRedis){
      await telRedis.set(telRedisKey('data.json'), payload);
      if(typeof telRedisVersionKey === 'function'){
        await telRedis.set(telRedisVersionKey(), `${Date.now()}-${crypto.randomBytes(4).toString('hex')}`);
      }
    }
  }catch(error){
    console.error('[strict-calendar] No se pudo guardar en Redis:', error);
  }
}
function telStrictCalPatchPayload(payload){
  try{
    if(!payload || typeof payload !== 'object') return payload;

    if(Array.isArray(payload)){
      telStrictCalFixData({competiciones:payload});
      return payload;
    }

    if(payload.data && typeof payload.data === 'object'){
      telStrictCalFixData(payload.data);
    }

    if(payload.competiciones || payload.ligas || payload.torneos){
      telStrictCalFixData(payload);
    }

    if(payload.competicion && typeof payload.competicion === 'object'){
      telStrictCalFixData({competiciones:[payload.competicion]});
    }
...
/* Desactiva forzadores viejos que volvían a meter J1/J2 o regeneraban mal. */
...
try{ telS3FixData = function(data){ return data; }; }catch(e){}

/* Filtro final de salida: aunque otra función antigua toque datos antes del res.json,
   justo antes de enviar se corrige otra vez. */
app.use((req,res,next)=>{
  const originalJson = res.json.bind(res);
  res.json = function(payload){
    return originalJson(telStrictCalPatchPayload(payload));
  };
  next();
});

app.get('/api/admin/arreglar-cruces-repetidos-definitivo', requireAdmin, async (req,res)=>{
  try{
    const data = readJson(DATA_FILE, {clubes:[], competiciones:[]});
    telStrictCalFixData(data);
    await telStrictCalHardSave(data);

    const resumen = (data.competiciones || data.ligas || data.torneos || []).map(comp=>({
      competicion: comp.nombre || comp.name || comp.id || 'Competición',
      cambios: comp._strictCrucesCambiados || [],
      problemas: comp._strictCrucesProblemas || [],
      sinSolucion: comp._strictCrucesSinSolucion || []
    }));

    res.set('Cache-Control','no-store');
    res.json({
      ok:true,
      message:'Cruces repetidos corregidos definitivamente. Si problemas sale [], ya no queda ningún partido más de 2 veces.',
      resumen
    });
  }catch(error){
    console.error('[arreglar-cruces-repetidos-definitivo]', error);
...
/* ============================================================
   TEL FIX DEFINITIVO CLASIFICACIÓN
   La clasificación sale SOLO de comp.equipos actual.
...
function telCompTableIsRemoved(team){
  ...
  const estado = telCompTableNorm(team.estado || team.status || '');
...
function telCompTableIsCup(comp){
  const txt = telCompTableNorm(`${comp?.nombre || ''} ${comp?.tipo || ''} ${comp?.formato || ''} ${comp?.formatoNombre || ''}`);
  if(txt.includes('copa') || txt.includes('elimin')) return true;
  return (comp?.partidos || []).some(match=>{
    const phase = telCompTableNorm(`${match?.rondaNombre || ''} ${match?.fase || ''}`);
    return phase.includes('cuarto') || phase.includes('semi') || phase.includes('final');
  });
}
function telCompTableScoreOk(value){
  return value !== null && value !== undefined && value !== '' && !Number.isNaN(Number(value));
}
function telCompTablePlayed(match){
  return !!match && (
    match.finalizado === true ||
    ['finalizado','jugado','completado'].includes(String(match.estado || '').toLowerCase()) ||
    (telCompTableScoreOk(match.localGoles) && telCompTableScoreOk(match.visitanteGoles)) ||
    (telCompTableScoreOk(match.golesLocal) && telCompTableScoreOk(match.golesVisitante)) ||
    /\d+\s*[-:]\s*\d+/.test(String(match.resultado || ''))
  );
}
function telCompTableGoals(match){
...
function telCompTableBuildActiveIndexes(comp){
  ...
  teams.forEach((team, index)=>{
    if(telCompTableIsRemoved(team)) return;

    const name = telCompTableName(team);
    if(!name) return;

    const slotId = telCompTableSlot(team, index);
    activeNames.add(telCompTableNorm(name));
  ...
  return {rowsBySlot, activeNames};
}
function telCompTableRecalc(comp){
  if(!comp || telCompTableIsCup(comp)) return comp;

  comp.equipos = Array.isArray(comp.equipos) ? comp.equipos : [];
  comp.partidos = Array.isArray(comp.partidos) ? comp.partidos : [];

  const {rowsBySlot, activeNames} = telCompTableBuildActiveIndexes(comp);

  comp.partidos.forEach(match=>{
    if(!telCompTablePlayed(match)) return;

    const localSlot = String(match.localSlotId || '');
    const awaySlot = String(match.visitanteSlotId || '');

    let local = rowsBySlot.get(localSlot) || null;
    let away = rowsBySlot.get(awaySlot) || null;

    // Fallback por nombre, pero SOLO si el nombre existe en comp.equipos activo.
    if(!local){
      const localName = telCompTableNorm(match.localNombre || match.nombreLocal || match.equipoLocal || '');
      if(activeNames.has(localName)){
        for(const row of rowsBySlot.values()){
          if(telCompTableNorm(row.nombre || row.clubNombre) === localName){
            local = row;
            break;
          }
        }
    ...
    if(!away){
      const awayName = telCompTableNorm(match.visitanteNombre || match.nombreVisitante || match.equipoVisitante || '');
      if(activeNames.has(awayName)){
        for(const row of rowsBySlot.values()){
          if(telCompTableNorm(row.nombre || row.clubNombre) === awayName){
            away = row;
            break;
          }
        }
    ...
    // Si un equipo ya no está en comp.equipos activo, no vuelve a tabla.
    // Si el rival sí está activo, conserva su resultado.
    if(!local && !away) return;

    const goals = telCompTableGoals(match);
...
/* Sobrescribe recalculos anteriores para que todos usen comp.equipos actual. */
function telPanelTeamsApply(data){ return telCompTableApply(data); }
function telCleanRemovedApplyAll(data){ return telCompTableApply(data); }
function telOnly10Apply(data){ return telCompTableApply(data); }

app.get('/api/admin/recalcular-clasificacion-competicion', requireAdmin, (req,res)=>{
  try{
    const data = readJson(DATA_FILE, {clubes:[], competiciones:[]});
    ...
    telCompTableApply(data);
    telCompTableSave(data);

    const resumen = (data.competiciones || data.ligas || data.torneos || []).map(comp=>({
      competicion: comp.nombre || comp.name || comp.id || 'Competición',
      equiposEnPanel: Array.isArray(comp.equipos) ? comp.equipos.filter(t=>!telCompTableIsRemoved(t)).length : 0,
    ...
    res.json({
      ok:true,
      message:'Clasificación recalculada solo con equipos actuales de la competición.',
      resumen
    });
  }catch(error){
    console.error('[recalcular-clasificacion-competicion]', error);
    res.status(500).json({ok:false,message:String(error.message || error)});
  }
});
...
/* ============================================================
   TEL FIX SEGURO: SIN FLASH DEMO, SIN OCULTAR TODA LA WEB
   - No inyecta script visual agresivo.
   - No borra tarjetas por texto en el navegador.
   - Solo limpia equipos/noticias demo dentro de la data JSON.
   ============================================================ */
function telSafeDemoNorm(value){
  return String(value || '')
    .toLowerCase()
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g,'')
...
const TEL_SAFE_DEMO_TEAMS = new Set([
  'thunder wolves',
  'delta united',
  'legends fc',
  'royal fc',
...
const TEL_SAFE_DEMO_NEWS = [
  'bienvenidos a thunder elite league',
  'comienza la temporada 1',
  'reglamento oficial de la liga',
  'conoce todos los detalles de la temporada 1',
  'calendario formato de competicion y premios',
  'consulta todas las normas y requisitos'
];
function telSafeDemoTeamName(team){
  return String(team?.nombre || team?.clubNombre || team?.nombreVisual || team?.name || team?.equipo || '').trim();
}
function telSafeDemoIsTeam(team){
  if(!team) return false;
  const name = telSafeDemoNorm(telSafeDemoTeamName(team));
  if(TEL_SAFE_DEMO_TEAMS.has(name)) return true;

  const values = [team.id, team.clubId, team.slotId, team.idClub, team.rolId]
    .filter(Boolean)
    .map(telSafeDemoNorm);

  return values.some(value => TEL_SAFE_DEMO_TEAMS.has(value));
}
function telSafeDemoIsMatch(match){
  if(!match) return false;
  const names = [
    match.localNombre,
    match.nombreLocal,
  ...
  return names.some(name => TEL_SAFE_DEMO_TEAMS.has(telSafeDemoNorm(name)));
}
function telSafeDemoIsNews(item){
  if(!item) return false;
  const text = telSafeDemoNorm(`${item.titulo || item.title || ''} ${item.descripcion || item.description || ''} ${item.texto || item.contenido || item.body || ''}`);
  return TEL_SAFE_DEMO_NEWS.some(value => text.includes(value));
}
function telSafeDemoCleanData(data){
  if(!data || typeof data !== 'object') return data;

  for(const key of ['clubes','equipos','teams']){
    if(Array.isArray(data[key])){
      data[key] = data[key].filter(team => !telSafeDemoIsTeam(team));
    }
  }

  for(const key of ['noticias','news','ultimasNoticias','latestNews']){
    if(Array.isArray(data[key])){
      data[key] = data[key].filter(item => !telSafeDemoIsNews(item));
    }
  }

  for(const key of ['proximosPartidos','ultimosResultados','partidos','matches']){
    if(Array.isArray(data[key])){
      data[key] = data[key].filter(match => !telSafeDemoIsMatch(match));
    }
  }
  ...
  comps.forEach(comp => {
    ...
    for(const key of ['equipos','clubes','teams']){
      if(Array.isArray(comp[key])){
        comp[key] = comp[key].filter(team => !telSafeDemoIsTeam(team));
      }
    }

    for(const key of ['partidos','matches','proximosPartidos','ultimosResultados']){
      if(Array.isArray(comp[key])){
        comp[key] = comp[key].filter(match => !telSafeDemoIsMatch(match));
      }
    }

    if(Array.isArray(comp.clasificacion)){
      comp.clasificacion = comp.clasificacion.filter(row => !telSafeDemoIsTeam(row));
    }

    for(const key of ['noticias','news']){
      if(Array.isArray(comp[key])){
        comp[key] = comp[key].filter(item => !telSafeDemoIsNews(item));
      }
    }
...
/* Ruta manual segura */
app.get('/api/admin/fix-demo-seguro', requireAdmin, (req,res)=>{
  try{
    const data = readJson(DATA_FILE, {clubes:[], competiciones:[]});
    telSafeDemoCleanData(data);
    fs.writeFileSync(DATA_FILE, JSON.stringify(data, null, 2), 'utf8');
    res.set('Cache-Control','no-store');
    res.json({ok:true,message:'Demo/fantasma limpiada de forma segura sin ocultar la web.'});
  }catch(error){
    res.status(500).json({ok:false,message:String(error.message || error)});
  }
});
...
/* Protección segura: antes de guardar data.json limpia solo JSON demo/fantasma */
if(!global.__TEL_SAFE_DEMO_WRITE_PATCHED__){
  global.__TEL_SAFE_DEMO_WRITE_PATCHED__ = true;
  const __telSafeDemoOriginalWrite = fs.writeFileSync.bind(fs);
  let __telSafeDemoInside = false;

  fs.writeFileSync = function(filePath, data, ...args){
    try{
      if(!__telSafeDemoInside && path.resolve(String(filePath || '')) === path.resolve(DATA_FILE)){
        const parsed = JSON.parse(Buffer.isBuffer(data) ? data.toString('utf8') : String(data));
        if(parsed && typeof parsed === 'object' && typeof telSafeDemoCleanData === 'function'){
          telSafeDemoCleanData(parsed);
          data = JSON.stringify(parsed, null, 2);
        }
      }
    }catch(error){}

    __telSafeDemoInside = true;
    try{
      return __telSafeDemoOriginalWrite(filePath, data, ...args);
    }finally{
      __telSafeDemoInside = false;
    }
  };
}
...
/* ============================================================
   TEL CALENDARIO FINAL: REPARAR SIN BORRAR PARTIDOS
   Regla correcta para 10 equipos / 18 jornadas:
   - 5 partidos por jornada.
   - Ningún equipo juega dos veces en la misma jornada.
   - Cada cruce máximo 2 veces en toda la liga.
   - Si aparece una tercera vez, se cambia el rival por otro válido.
   - No elimina partidos.
   - Conserva resultados ya jugados.
   ============================================================ */
function telFinalCalNorm(value){
  return String(value || '')
    .toLowerCase()
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g,'')
...
function telFinalCalSlug(value){
  return String(value || 'equipo')
    .toLowerCase()
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g,'')
...
function telFinalCalIsCup(comp){
  const txt = telFinalCalNorm(`${comp?.nombre || ''} ${comp?.tipo || ''} ${comp?.formato || ''} ${comp?.formatoNombre || ''}`);
  if(txt.includes('copa') || txt.includes('elimin')) return true;

  return (comp?.partidos || []).some(match=>{
    const phase = telFinalCalNorm(`${match?.rondaNombre || ''} ${match?.fase || ''}`);
    return phase.includes('cuarto') || phase.includes('semi') || phase.includes('final');
  });
}
function telFinalCalTeamName(team){
  return String(team?.nombre || team?.clubNombre || team?.nombreVisual || team?.name || '').trim();
}
function telFinalCalIsRemoved(team){
  if(!team) return true;
  if(team.historico === true) return true;
  if(team.equipoHistorico === true) return true;
  if(team.fixtureHistorico === true) return true;
  ...
  const estado = telFinalCalNorm(team.estado || team.status || '');
  if(estado.includes('sacado')) return true;
  if(estado.includes('eliminado')) return true;
  if(estado.includes('inactivo')) return true;
  if(estado.includes('fuera')) return true;
...
function telFinalCalActiveTeams(comp){
  const seen = new Set();
  const result = [];

  for(const raw of (Array.isArray(comp?.equipos) ? comp.equipos : [])){
    if(telFinalCalIsRemoved(raw)) continue;

    const name = telFinalCalTeamName(raw);
    const key = telFinalCalNorm(name);
    if(!key || seen.has(key)) continue;
    ...
    if(!team.slotId) team.slotId = String(team.id || team.clubId || team.idClub || telFinalCalSlug(name));
    if(!team.id) team.id = String(team.slotId || team.clubId || telFinalCalSlug(name));
...
function telFinalCalMatchName(match, side){
  if(side === 'local'){
    return String(
      match?.localNombre ||
      match?.nombreLocal ||
...
function telFinalCalPairKey(a,b){
  const x = telFinalCalNorm(a);
  const y = telFinalCalNorm(b);
  if(!x || !y) return '';
  return [x,y].sort().join(' vs ');
}
function telFinalCalPairKeyMatch(match){
  return telFinalCalPairKey(telFinalCalMatchName(match,'local'), telFinalCalMatchName(match,'visitante'));
}
function telFinalCalPlayed(match){
  if(!match) return false;
  const hasScore = [match.localGoles, match.visitanteGoles, match.golesLocal, match.golesVisitante].some(value =>
...
function telFinalCalInject(match, side, team){
  const name = telFinalCalTeamName(team);
  const slot = String(team.slotId || team.id || team.clubId || team.idClub || telFinalCalSlug(name));
  const logo = telFinalCalLogo(team);
  const p = side === 'local' ? 'local' : 'visitante';
  const cap = side === 'local' ? 'Local' : 'Visitante';
...
function telFinalCalCount(countByPair, a, b){
  return countByPair.get(telFinalCalPairKey(a,b)) || 0;
}
function telFinalCalFindTeam(activeTeams, name){
  const key = telFinalCalNorm(name);
  return activeTeams.find(team=>telFinalCalNorm(telFinalCalTeamName(team)) === key) || null;
}
function telFinalCalPickPair(activeTeams, usedThisRound, countByPair, preferredA, preferredB){
  const prefA = preferredA ? telFinalCalFindTeam(activeTeams, preferredA) : null;
  const prefB = preferredB ? telFinalCalFindTeam(activeTeams, preferredB) : null;

  function usable(team){
    if(!team) return false;
    return !usedThisRound.has(telFinalCalNorm(telFinalCalTeamName(team)));
  }

  // 1) Mantener local si se puede y buscarle otro visitante.
  if(usable(prefA)){
    const aName = telFinalCalTeamName(prefA);
    for(const maxCount of [0,1]){
      const rival = activeTeams.find(team=>{
        if(!usable(team)) return false;
        const bName = telFinalCalTeamName(team);
        if(telFinalCalNorm(aName) === telFinalCalNorm(bName)) return false;
        return telFinalCalCount(countByPair, aName, bName) <= maxCount;
      });
      if(rival) return [prefA, rival];
    }
  }

  // 2) Mantener visitante si se puede y buscarle otro local.
  if(usable(prefB)){
    const bName = telFinalCalTeamName(prefB);
    for(const maxCount of [0,1]){
      const rival = activeTeams.find(team=>{
        if(!usable(team)) return false;
        const aName = telFinalCalTeamName(team);
        if(telFinalCalNorm(aName) === telFinalCalNorm(bName)) return false;
        return telFinalCalCount(countByPair, aName, bName) <= maxCount;
      });
      if(rival) return [rival, prefB];
    }
  }

  // 3) Cualquier pareja disponible.
  for(const maxCount of [0,1]){
    for(let i=0;i<activeTeams.length;i++){
      ...
      for(let j=i+1;j<activeTeams.length;j++){
        ...
        const aName = telFinalCalTeamName(a);
        const bName = telFinalCalTeamName(b);
        if(telFinalCalCount(countByPair, aName, bName) <= maxCount){
          return [a,b];
        }
      }
    }
...
function telFinalCalRepairCompetition(comp){
  if(!comp || telFinalCalIsCup(comp)) return comp;
  if(!Array.isArray(comp.partidos)) return comp;

  const activeTeams = telFinalCalActiveTeams(comp);
  if(activeTeams.length < 2) return comp;

  const matches = telFinalCalSort(comp.partidos.slice());
  ...
  function roundUsed(jornada){
    const key = String(Number(jornada || 0));
    if(!usedByRound.has(key)) usedByRound.set(key, new Set());
    return usedByRound.get(key);
  }
  function addMatchCount(match){
    const local = telFinalCalMatchName(match,'local');
    const away = telFinalCalMatchName(match,'visitante');
    const pair = telFinalCalPairKey(local, away);
    if(pair) countByPair.set(pair, (countByPair.get(pair) || 0) + 1);
    const used = roundUsed(match.jornada || match.round || 0);
    if(local) used.add(telFinalCalNorm(local));
    if(away) used.add(telFinalCalNorm(away));
  }
  ...
  for(const match of matches){
    ...
    const away = telFinalCalMatchName(match,'visitante');
    const pair = telFinalCalPairKey(local, away);
    const used = roundUsed(jornada);
    const localUsed = local && used.has(telFinalCalNorm(local));
    const awayUsed = away && used.has(telFinalCalNorm(away));
    const pairCount = pair ? (countByPair.get(pair) || 0) : 0;
    const played = telFinalCalPlayed(match);

    // Resultados jugados se conservan siempre, aunque haya duplicados antiguos.
    if(played){
      addMatchCount(match);
      fixed.push(match);
      continue;
    ...
    // Si todavía no rompe reglas, se queda.
    if(pair && pairCount < 2 && !localUsed && !awayUsed){
      addMatchCount(match);
      fixed.push(match);
      continue;
    }

    // Si rompe reglas, se reasigna sin borrar.
    const chosen = telFinalCalPickPair(activeTeams, used, countByPair, local, away);

    if(chosen){
      const oldLocal = local;
      const oldAway = away;
      telFinalCalInject(match, 'local', chosen[0]);
      telFinalCalInject(match, 'visitante', chosen[1]);
      match.reparadoCalendario = true;
      match.cruceAnterior = `${oldLocal} vs ${oldAway}`;
      match.cruceNuevo = `${telFinalCalTeamName(chosen[0])} vs ${telFinalCalTeamName(chosen[1])}`;
      addMatchCount(match);
      fixed.push(match);
      cambios.push({
        jornada,
        antes:`${oldLocal} vs ${oldAway}`,
        ahora:`${telFinalCalTeamName(chosen[0])} vs ${telFinalCalTeamName(chosen[1])}`,
        motivo: pairCount >= 2 ? 'tercer cruce repetido' : 'equipo repetido en la misma jornada'
      });
      continue;
    }

    // Último caso: no había forma válida. Se conserva pero queda marcado.
    match.necesitaRevisionCalendario = true;
    addMatchCount(match);
    fixed.push(match);
    revisiones.push({
      jornada,
      partido:`${local} vs ${away}`,
      motivo:'No se encontró rival activo disponible sin romper reglas'
    });
  }

  comp.partidos = telFinalCalSort(fixed);
  comp._calendarioReparadoCambios = cambios;
  comp._calendarioReparadoRevision = revisiones;
...
function telFinalCalRepairData(data){
  if(!data || typeof data !== 'object') return data;

  const comps = data.competiciones || data.ligas || data.torneos || [];
  comps.forEach(comp=>telFinalCalRepairCompetition(comp));
...
/* Desactivar limpiadores antiguos que borraban partidos */
...
app.get('/api/admin/reparar-calendario-final', requireAdmin, (req,res)=>{
  try{
    const data = readJson(DATA_FILE, {clubes:[], competiciones:[]});
    ...
    const resumen = (data.competiciones || data.ligas || data.torneos || []).map(comp=>({
      ...
      cambios: Array.isArray(comp._calendarioReparadoCambios) ? comp._calendarioReparadoCambios : [],
      revisar: Array.isArray(comp._calendarioReparadoRevision) ? comp._calendarioReparadoRevision : []
    }));

    res.set('Cache-Control','no-store');
    res.json({
      ok:true,
      message:'Calendario reparado sin borrar partidos: terceros cruces reasignados a rivales válidos.',
      resumen
    });
  }catch(error){
    console.error('[reparar-calendario-final]', error);
    res.status(500).json({ok:false,message:String(error.message || error)});
  }
});
...
/* Protección final calendario: repara cruces duplicados sin borrar antes de guardar */
if(!global.__TEL_FINAL_CALENDAR_REPAIR_WRITE_PATCHED__){
  global.__TEL_FINAL_CALENDAR_REPAIR_WRITE_PATCHED__ = true;
  const __telFinalCalendarOriginalWrite = fs.writeFileSync.bind(fs);
  let __telFinalCalendarInside = false;

  fs.writeFileSync = function(filePath, data, ...args){
    try{
      if(!__telFinalCalendarInside && path.resolve(String(filePath || '')) === path.resolve(DATA_FILE)){
        const parsed = JSON.parse(Buffer.isBuffer(data) ? data.toString('utf8') : String(data));
        ...
        if(parsed && typeof parsed === 'object'){
          ...
          if(typeof telFinalCalRepairData === 'function'){
            telFinalCalRepairData(parsed);
          }
          data = JSON.stringify(parsed, null, 2);
        }
      }
    ...
    __telFinalCalendarInside = true;
    try{
      return __telFinalCalendarOriginalWrite(filePath, data, ...args);
    }finally{
      __telFinalCalendarInside = false;
    }
  };
}
...
/* TEL STRICT CALENDAR FINAL OVERRIDE - repetir desactivación al final */
try{ telLock12Apply = function(data){ return data; }; }catch(e){}
try{ telForceFirstTwoMatchdays = function(data){ return false; }; }catch(e){}
try{ telFotoExactApply = function(data){ return data; }; }catch(e){}
try{ telOVRebuildData = function(data){ return data; }; }catch(e){}
